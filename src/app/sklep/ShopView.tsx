...
/** Metadane dla strony kategorii (czysty adres /sklep/kategoria/<slug>). */
export async function categoryMetadata(slug: string): Promise<Metadata> {
  ...
  const desc =
    (cat.meta_description as string | null) ||
    (cat.description as string | null) ||
    undefined;
  const imgUrl = cat.image_url as string | null;
  return {
    title: `${cat.name as string} | Sklep`,
    description: desc,
    alternates: { canonical: `/sklep/kategoria/${slug}` },
    openGraph: {
      title: `${cat.name as string} | Sklep`,
      description: desc,
      url: `/sklep/kategoria/${slug}`,
...
/**
 * Wspólny widok sklepu. `category` pochodzi z czystego adresu
 * (/sklep/kategoria/<slug>) albo jest null dla /sklep (wszystkie produkty).
 */
